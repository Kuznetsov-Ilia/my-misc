import { head, window } from 'my-global';
...
function param(data) {
  if (isObject(data)) {
    return keys(data).map(key => `${encodeURIComponent(key)}=${encodeURIComponent(data[key])}`).join('&').replace(r20, '+');
  } else {
    return '';
  }
}
...
function scriptify(HTMLElement) {
  ...
  var text = internal_scripts.map(script => script.text || script.textContent || script.innerHTML || '').join(';\n');
...
export { assign, clone, contains, debounce, decl, diff, encode, extend, inherits, is, isArray, isEmpty, isEqual, isFragment, isFunction, isNode, isNumber, isObject, isRegExp, isset, isString, isUndefined, keys, loader, noop, now, param, pick, rand, result, scriptify, send, stringHash, strip_tags, throttle, timestamp, toArray, toRadix };
